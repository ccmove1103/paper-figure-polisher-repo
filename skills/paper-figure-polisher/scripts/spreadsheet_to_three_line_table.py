...
def parse_args():
    ...
    parser.add_argument(
        "--input-docx",
        help="Optional existing .docx to insert the generated table into",
    )
...
def choose_insertion_target(document):
    ...
    first_figure_index = len(blocks)
    last_table_index = None
    for index, (kind, obj) in enumerate(blocks):
        if kind == "paragraph" and has_drawing(obj):
            first_figure_index = index
            break
        if kind == "table":
            last_table_index = index

    if last_table_index is None:
        for index in range(first_figure_index):
            if blocks[index][0] == "table":
                last_table_index = index

    if last_table_index is not None:
        end_index = last_table_index
        while end_index + 1 < first_figure_index:
            next_kind, next_obj = blocks[end_index + 1]
            if next_kind != "paragraph" or not NOTE_RE.match(next_obj.text.strip()):
                break
            end_index += 1
        return "after", blocks[end_index][1]

    if first_figure_index < len(blocks):
        return "before", blocks[first_figure_index][1]
...
def relocate_blocks(blocks, placement, anchor):
    ...
    if placement == "before":
        anchor_element = block_element(anchor)
        for block in blocks:
            anchor_element.addprevious(block_element(block))
        return
...
def build_table(
    ...
    if caption:
        ...
        created_blocks.append(paragraph)

    table = document.add_table(rows=1, cols=len(headers))
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    table.style = "Table Grid"
    created_blocks.append(table)
    ...
    if note:
        ...
        created_blocks.append(note_paragraph)
...
def main():
    ...
    placement, anchor = choose_insertion_target(document) if args.input_docx else ("append", None)
    created_blocks = build_table(
        document,
        grouped_rows,
        headers,
        args.caption,
